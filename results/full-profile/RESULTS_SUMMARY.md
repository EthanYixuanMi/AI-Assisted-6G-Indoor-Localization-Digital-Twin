...
- 独立仿真与训练 seed 的 Normal 空间留出结果中，平均误差最低的是 **Residual AI**：
  **3.90 m**，P90 为 **7.97 m**。
- Residual AI 相对 Geometric LS 的平均误差改善为 27.1%；该结论仅适用于保存的配置和空间留出协议。
- LoS 条件下最优方法为 **Residual AI**
  （3.05 m），NLoS 条件下最优方法为
  **Residual AI**（4.13 m）。
- 推理最快的是 **KNN**，本机预热后一行输入时间为
  **1.612 ms**。
